...
void FPSProfiler::tick() {
    ...
    if (!started) {
        ...
        cur_fps = fps;
        if (printLog) {
            qDebug().noquote() << QString("[%1] FPS profiling: %2 events / %3 s.  fps: %4  since last: %5 s")
                .arg(title).arg(tick_count).arg(elapsed_since_start_s).arg(fps).arg(elapsed_since_last_s);
        }
    }
    last_time = now;
}
